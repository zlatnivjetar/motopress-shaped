...
class OAuthService extends \MPHB\Stripe\Service\AbstractService
{
    /**
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \MPHB\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \MPHB\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \MPHB\Stripe\Util\RequestOptions::parse($opts);
    }
    /**
