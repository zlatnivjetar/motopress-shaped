...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = ['api_key' => null, 'client_id' => null, 'stripe_account' => null, 'stripe_version' => \MPHB\Stripe\Util\ApiVersion::CURRENT, 'api_base' => self::DEFAULT_API_BASE, 'connect_base' => self::DEFAULT_CONNECT_BASE, 'files_base' => self::DEFAULT_FILES_BASE];
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        $config = \array_merge(self::DEFAULT_CONFIG, $config);
        $this->validateConfig($config);
        $this->config = $config;
        $this->defaultOpts = \MPHB\Stripe\Util\RequestOptions::parse(['stripe_account' => $config['stripe_account'], 'stripe_version' => $config['stripe_version']]);
    }
    /**
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $requestor = new \MPHB\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        if (!$obj instanceof \MPHB\Stripe\Collection) {
            ...
            throw new \MPHB\Stripe\Exception\UnexpectedValueException($msg);
        }
    ...
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \MPHB\Stripe\Exception\AuthenticationException($msg);
        }
        return $apiKey;
    }
    /**
    ...
    private function validateConfig($config)
    {
        ...
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }
        if (null !== $config['api_key'] && '' === $config['api_key']) {
            $msg = 'api_key cannot be the empty string';
            throw new \MPHB\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }
        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }
        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }
        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }
        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }
        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \MPHB\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
}
