...
## Separate Task: Make Checkout Datepicker Open at the Same Month as the Selected Check-In Date

### Problem

When a user selects a check-in date far in the future (e.g. June 2026 while today is February 2026), the checkout datepicker auto-opens but lands on the current month (February) instead of June. The user has to click "next" multiple times to navigate back to the month they just selected.

### Root Cause

In `vendors/kbwood/datepick/jquery.datepick.js`, the `show()` method determines which month to display via:

```js
inst.drawDate = plugin._checkMinMax(plugin.newDate(
    inst.selectedDates[0] || inst.get('defaultDate') || plugin.today()
), inst);
```

Since the Shaped theme clears the checkout value before opening it, `selectedDates[0]` is empty. The `defaultDate` falls back to `firstAvailableCheckInDate` (today or near today), so the calendar opens at today's month.

### Fix

In `assets/js/public/mphb.js`, inside the `SearchCheckInDatepicker`'s `onSelect` callback, set the checkout datepicker's `defaultDate` option to the selected check-in date **before** calling `datepick('show')`:

```js
// Inside onSelect of SearchCheckInDatepicker:
if (dates[0]) {
    self.form.checkOutDatepicker.element.datepick('option', 'defaultDate', dates[0]);
}
```

This single line makes `show()` use the check-in date for `drawDate`, so the checkout calendar opens at the same month the user just selected.

### Where Exactly to Place It

In `assets/js/public/mphb.js`, find the `onSelect` handler of `MPHB.SearchCheckInDatepicker`. The `datepick('option', ...)` call goes after the checkout value is cleared and before the `setTimeout` that calls `datepick('show')`. The full block should look like:
...
onSelect: function onSelect(dates) {
    ...
    if (self.form && self.form.checkOutDatepicker) {
        ...
        // Set checkout default date to the selected check-in date
